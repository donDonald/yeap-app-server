...
class Application {
    ...
    loadPlaces (cb) {
        ...
        const loadFiles = (index, files, place, cb)=>{
            if (index < files.length) {
                ...
                let relPath='';
                place.forEach((p)=>{
                    relPath = api.lib.path.join(relPath, p);
                });
                relPath = api.lib.path.join(relPath, file);

                const path = api.lib.env.makePath(relPath);
                const stat = api.lib.fs.lstatSync(path);
                if (stat.isDirectory()) {
                    api.lib.fs.readdir(path, (err, files2) => {
                        if (err) {
        ...
        const loadPlaces = (index, places, cb)=>{
            if (index < places.length) {
                ...
                api.lib.fs.readdir(path, (err, files) => {
                    if (err) {
    ...
    loadCertificatesIfAny (config) {
        ...
        if (config && config.key && config.cert) {
            ...
            try {
                key = api.lib.fs.readFileSync(keyFile);
                cert = api.lib.fs.readFileSync(certFile);
