...
Application.STATIC_APIS = ['fs', 'path'];
            ...
            api.fs.readdir(path, (err, files) => {
                if (err) {
                    ...
                    loadFiles(0, files, [place], (err)=>{
                        if (err) {
                            api.lib.log.ctx('Application').ctx('setup').error('Loading files has failed:' + err);
                        } else {
                            api.lib.log.ctx('Application').ctx('setup').info('Loading files is complete');
                        }
                        cb(err);
                    });
                }
...
Application.prototype._openModel = function(cb) {
    const api = this._api;
    api.lib.log.ctx('Application').ctx('setup').info('Opening model...');

    if (this.openModel) {
        this.openModel(api, (err)=>{
            assert(!err, err);
            api.lib.log.ctx('Application').ctx('setup').info('Opening model is complete');
            cb();
        });
    } else {
        api.lib.log.ctx('Application').ctx('setup').info('Opening model is complete, no model is set');
        setImmediate(cb);
    }
}
...
Application.prototype._closeModel = function(cb) {
    const api = this._api;
    api.lib.log.ctx('Application').ctx('teardown').info('Closing model...');

    if (this.closeModel) {
        this.closeModel(api, (err)=>{
            assert(!err, err);
            api.lib.log.ctx('Application').ctx('teardown').info('Closing model is complete');
            cb();
        });
    } else {
        api.lib.log.ctx('Application').ctx('teardown').info('Closing model is complete, no model is set');
        setImmediate(cb);
    }
}
...
Application.assertAppIsClosed = function(app) {
    ...
    assert.equal('undefined', typeof app._httpServer);
}
...
Application.assertAppIsOpen = function(app) {
    ...
    assert.notEqual('undefined', typeof app._httpServer);
}
