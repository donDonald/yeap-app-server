...
describe('yeap_app_server.auth.providers.bogus.ProviderToken', ()=>{
    ...
    before(()=>{
        api = re('index');
        createDbName=(name)=>{ return api.db.Db.createDbName('yeap_app_server_auth_userhandler_') + name };
        api.app_server.auth = {};
        api.app_server.auth.UserHandler = re('auth/UserHandler')(api);
        api.app_server.auth.providers = {};
        api.app_server.auth.providers.bogus = {};
        api.app_server.auth.providers.bogus.ProviderToken = re('auth/providers/bogus/ProviderToken')(api);

        // Database record
        User.makeId = api.makeId;
        User.makeCat = api.makeId;
    ...
    describe('#Provider.collectUser()', ()=>{
        it('collect user', ()=>{
            const provider = new api.app_server.auth.providers.bogus.ProviderToken({});
            const profile = {
                id: 'someId',
    ...
    describe('#Provider.handleUser()', ()=>{
        ...
        before((done)=>{
            const database = createDbName('handleuser');
        ...
        it('Create Users container', (done)=>{
            ...
            Users.create = function (dbProps, cb) {
            ...
            Users.prototype.init = function(cb) {
                api.db.postgres.Pool.create(this.dbProps, this.dbProps.database, (err, dbc)=>{
                    this.dbc = dbc;
                    this.add = api.db.model.add.bind(this, this.dbc, this.tableName, User);
                    ...
                    this.query = api.db.model.query.run.bind(this, this.dbc, this.tableName, User, this.inserter);
        ...
        let provider;
        it('Create Provider', ()=>{
            provider = new api.app_server.auth.providers.bogus.ProviderToken(users, User);
