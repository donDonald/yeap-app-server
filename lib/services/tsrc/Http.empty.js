...
describe('yeap_app_server.services.Http, Empty service, root route only, no SSL, no access.js, no validator.js', function() {
    ...
    before(()=>{
        api = re('index');
        getRoutes = api.dev_tools.express.getRoutes;
    ...
    before(()=>{
        process.env.APP_ROOT = __dirname + '/test.Http.empty';
    });
    ...
    describe('Start and stop', ()=>{
        ...
        before((done)=>{
            ...
            loggerFs = helpers.loggerSetup(process.env.APP_ROOT);
            done();
        });
        after((done)=>{
        ...
        it('Query, url: /notExistingRoute', (done)=>{
            ajax.get(SESSION, DOMAIN, 'notExistingRoute', {}, {}, function(err, session, result) {
                //console.log('err:', err);
