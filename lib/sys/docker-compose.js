...
module.exports = function(api) {
    ...
    class Container {
        static create(line) {
//          api.log.debug('Container.create(), line:' + line);
            let self;
            //const columns = line.split(/(\s+)/);
            //const columns = line.split(/(\s\s+)/);
            const columns = line.split(/(\s\s+)/).filter( function(e) { return e.trim().length > 0; } );
//          api.log.debug('Container.create, columns:'); console.log(columns);
            if (columns.length > 0) {
                self = new Container(columns);
            }
//          api.log.debug('Container.create, container:'); console.log(self);
            return self;
        }
    ...
    impl.ps = function(cb) {
//      api.log.debug('docker-compose.ps(), context:' + impl.context);
        assert(cb);
...
//          api.log.debug('docker-compose.ps, stdout:' + stdout);
//          api.log.debug('docker-compose.ps, stderr:' + stderr);
            if (err) {
                cb(err);
            } else {
                const containers = impl.ps_handler(stdout);
//              api.log.debug('docker-compose.ps, containers:'); console.log(containers);
...
//      api.log.debug('docker-compose.ps_handler(), stdout:' + stdout);
        const containers = {};
        const lines = stdout.split('\n');
        lines.forEach((line, index)=>{
//          api.log.debug('docker.ps_handler, line[' + index + ']:' + line);
            if (index>1) {
                const container = Container.create(line);
                if (container) containers[container.name] = container;
            }
        });
//      api.log.debug('docker-compose_ps.handler, containers:'); console.log(containers);
        return containers;
    }
...
//      api.log.debug('docker-compose.build(), context:' + impl.context);
        assert(cb);
        ...
        exec2(cmd, opts, function (err, stdout, stderr) {
//          api.log.debug('docker-compose.build, stdout:' + stdout);
//          api.log.debug('docker-compose.build, stderr:' + stderr);
            cb(err);
        });
    };
    ...
    impl.up = function(cb) {
//      api.log.debug('docker-compose.up(), context:' + impl.context);
        assert(cb);
        ...
        exec2(cmd, opts, function (err, stdout, stderr) {
//          api.log.debug('docker-compose.up, stdout:' + stdout);
//          api.log.debug('docker-compose.up, stderr:' + stderr);
            if (err) {
                cb(err);
            } else {
                impl.ps((err, containers)=>{
//                  api.log.debug('docker-compose.up, containers:'); console.log(containers);
                    cb(err, containers);
                });
            }
        });
...
//      api.log.debug('docker-compose.down(), context:' + impl.context);
        assert(cb);
        const self = this;
        ...
        exec2(cmd, opts, function (err, stdout, stderr) {
//          api.log.debug('docker-compose.down, stdout:' + stdout);
//          api.log.debug('docker-compose.down, stderr:' + stderr);
            if (err) {
                cb(err);
            } else {
                impl.ps((err, containers)=>{
//                  api.log.debug('docker-compose.down, containers:'); console.log(containers);
                    if (err) {
                        cb(err);
