...
describe('app_server.Application', function() {

    const assert = require('assert');
    const re = (module)=>{ return require('../' + module); }

    let Application, getRoutes, OLD_APP_ROOT;
    before(()=>{
        getRoutes = require('yeap_lib').lib.express.getRoutes;
        OLD_APP_ROOT = process.env.APP_ROOT;
        process.env.APP_ROOT = process.env.APP_ROOT + '/lib/tsrc/app1';
    ...
    describe('Prepare databases', ()=>{
        it('Create database', (done)=>{
            ...
            foreach(0, entries, (d, complete)=>{
                ...
                db[dbProps.schema].helpers.create(dbProps.database, (err)=>{
                    assert(!err);
                    complete();
                });
            }, done);
