...
describe('yeap_app_server.Application', ()=>{
    ...
    before(()=>{
        ...
        process.env.YEAP_APP_SERVER_ROOT = __dirname + '/app1';
    ...
    describe('Open app and close it', ()=>{
        ...
        it('Check g_application', ()=>{
            assert(!global.g_application, 'g_application shall not stick out of the sandbox');
        });
        ...
        it('Check g_application', ()=>{
            assert(!global.g_application, 'g_application shall not stick out of the sandbox');
            assert(app.sandbox.global.g_application);
            assert(app.sandbox.global.g_application.config);
            assert(app.sandbox.global.g_application.config.databases);
            assert(app.sandbox.global.g_application.config.services);
            assert(app.sandbox.global.g_application.config.services.http);
            assert(app.sandbox.global.g_application.model);
            assert(app.sandbox.global.g_application.services);
            assert(app.sandbox.global.g_application.services.http);
            assert.equal('http', app.sandbox.global.g_application.services.http.name);
            assert.equal(true, app.sandbox.global.g_application.services.http.isStarted);
        });
        ...
        it('#Verify model', ()=>{
            assert.equal('object', typeof app.model);
            assert.equal('object', typeof app.model.list);
        });
